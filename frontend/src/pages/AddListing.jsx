...
const AddListing = ()=>{
    ...
    async function registerNewListing(data){
        await axios.post("http://127.0.0.1:4000/api/v1/listings/addListing", data, {headers:{Authorization:`Bearer ${user.token}`}})
        .then((result)=>{
    ...
    async function editListing(data){
        await axios.put("http://127.0.0.1:4000/api/v1/listings/editListing/"+editListingID, data, {headers:{Authorization:`Bearer ${user.token}`}})
        .then((result)=>{
            navigate("../viewListing/"+result.data.data._id);
        })
        .catch((error)=>{console.log(error)});
    ...
    function handleSubmit(e){
        ...
        if(!filteredExistImg.length||!listingCategory){
            ...
            filteredExistImg.forEach((image)=>{data.append("listing-img", image)});
            if(!editMode){
    ...
    useEffect(()=>{
        if(location.state){
            ...
            setListingCategory(selectOptions[selectOptions.findIndex(option=>option.value==selectedListing.category)]);
            setEditMode(true);
    ...
    useEffect(()=>{console.log("NNN"+selectedImg1)},[selectedImg1])
    return (
        <form className="listingWindow materialWhite addListingDiv" onSubmit={(e)=>{handleSubmit(e)}}>
            <div className="imageInputContent">
                ...
                <label htmlFor="imgUpload1">
                    <div className="imgUploadDisplay materialWhite noPointers">
                        ...
                        {selectedImg1&&
                            <button className="imageRemoveButton havePointers" onClick={()=>{handleXImage(0)}}>X</button>
                        }
                    </div>
                </label>
                ...
                <label htmlFor="imgUpload2">
                    <div className="imgUploadDisplay materialWhite noPointers">
                        ...
                        {selectedImg2&&
                            <button className="imageRemoveButton havePointers" onClick={()=>{handleXImage(1)}}>X</button>
                        }
                    </div>
                </label>
                ...
                <label htmlFor="imgUpload3">
                    <div className="imgUploadDisplay materialWhite noPointers">
                        ...
                        {selectedImg3&&
                            <button className="imageRemoveButton havePointers" onClick={()=>{handleXImage(2)}}>X</button>
                        }
                    </div>
                </label>
                ...
                <label htmlFor="imgUpload4">
                    <div className="imgUploadDisplay materialWhite noPointers">
                        ...
                        {selectedImg4&&
                            <button className="imageRemoveButton havePointers" onClick={()=>{handleXImage(3)}}>X</button>
                        }
                    </div>
                </label>
            </div>
            ...
            <h2>Listing Name</h2>
            <input type="text" onChange={(e)=>{setListingName(e.target.value)}} value={listingName} required></input>
            <h3>Description</h3>
            <input type="text" onChange={(e)=>{setListingDescription(e.target.value)}} value={listingDescription} required></input>
            <h3>Price</h3>
            <input type="number" onChange={(e)=>{setListingPrice(e.target.value)}} value={listingPrice} required></input>
            <h3>Category</h3>
            {/* <select id="category">
             ...
             */}
             <div className="categorySelect">
                <Select value={listingCategory} options={selectOptions} onChange={(choice)=>{setListingCategory(choice); setCatErr(false)}} isSearchable={false}/>
             </div>
             {catErr&&
                <h3 className="error">Select a category</h3>
             }
            <button className="addButton" type="submit">{editMode?"Edit Listing":"Add a listing!"}</button>
        </form>
    );
}
